...
(function (root) {
    ...
    class App {
        ...
        async init() {
            ...
            if (!loaded || !this.doc || !this.doc.camera)
                this.fit(false);
            else
        ...
        switchDocument(id) { const prevDoc = this.doc; if (prevDoc)
            prevDoc.camera = this.camera.serialize(); this.cancel(false); this.activeId = id; const doc = this.doc; if (!doc)
        ...
        setView(name) { this.camera.setView(name); $('view-select').value = name; if (name !== 'top' && name !== 'bottom') {
            this.workspace = '3d';
            if (this.renderer.style === 'wireframe' && this.doc && this.doc.entities.some(e => e.type === 'MESH'))
                this.renderer.style = 'shaded-edges';
        }
        else if (name === 'top')
        ...
        cornerOperation(id, e1, e2, distance, pick1 = null, pick2 = null) { const hit = lineIntersection(...e1.points, ...e2.points); if (!hit)
            throw Error('Parallel lines do not define a corner.'); const p = hit.point, farthest = e => e.points.reduce((a, b) => V.dist(a, p) > V.dist(b, p) ? a : b), side = (e, pick) => { const p0 = e.points[0], p1 = e.points[1], d = V.sub(p1, p0), len = V.len(d); if (!(len > EPS))
                return p0; const dir = V.mul(d, 1 / len), sPick = V.dot(V.sub(pick, p), dir), s0 = V.dot(V.sub(p0, p), dir), s1 = V.dot(V.sub(p1, p), dir); if (Math.abs(sPick) < 1e-9)
                return farthest(e); const straddle = (s0 > 1e-9 && s1 < -1e-9) || (s0 < -1e-9 && s1 > 1e-9); if (straddle)
                return sPick > 0 ? (s0 > s1 ? p0 : p1) : (s0 < s1 ? p0 : p1); return farthest(e); }, far = (e, pick) => pick ? side(e, pick) : farthest(e), a = far(e1, pick1), b = far(e2, pick2), u = V.norm(V.sub(a, p)), v = V.norm(V.sub(b, p)), theta = Math.acos(clamp(V.dot(u, v), -1, 1)); if (theta < .001 || Math.abs(theta - Math.PI) < .001)
        ...
        selectWindow(a, b, add) {
            ...
            for (const { e, bounds: q, segments, faces, g } of this.index.items.values()) {
                ...
                if (!crossing)
                    ...
                    hit = segments.some(s => within(s[0]) || within(s[1]) || edges.some(edge => { const h = lineIntersection(...s, ...edge); return h && h.t >= 0 && h.t <= 1 && h.u >= 0 && h.u <= 1; })) || (g.texts ? g.texts.length > 0 : false) || (faces ? faces.some(f => inside([rect[0], rect[1]], f)) : false);
                }
                if (hit) {
                    ids.push(e.id);
                    if (e.group)
