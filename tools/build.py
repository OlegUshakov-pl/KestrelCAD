...
def build(output: Path) -> None:
    ...
    worker = '\n'.join((ROOT / 'src' / name).read_text(encoding='utf-8') for name in ('math.js','geometry.js','model.js','exchange.js','production.js','kernel.js', 'constraints.js','spatial-constraints.js','dynamic-blocks.js','fonts.js','unicode-bidi.js','mtext.js','source-document.js','productivity.js','fields.js'))
    worker += '\n' + re.sub(r"importScripts\([^;]+;", '', (ROOT / 'src/io-worker.js').read_text(encoding='utf-8'))
    script_data = json.dumps(worker).replace('</', '<\\/')
    html = html.replace('</head>', '<script>window.KESTREL_WORKER_SOURCE='+script_data+';</script>\n</head>')
    def inline(match):
        source = (ROOT / 'src' / match.group(1)).read_text(encoding='utf-8')
